...
/**
 * This code is a modified version of original code - "Alexa Skill Sample"
 * 
 * Github source : https://github.com/alexa/skill-sample-nodejs-trivia
 * 
 * License : https://github.com/alexa/skill-sample-nodejs-trivia/blob/master/LICENSE.txt
 * 
 * Modified by : Mariswaran for World language Trivia
 * Creation Data : June 2017 
 * 
 */

var APP_ID = "amzn1.ask.skill.49e9f032-aeb8-4e85-bacc-8471674e9d0d";
...
var languageString = {
    "en": {
        "translation": {
            ...
            "GAME_NAME" : "World language Trivia", // Be sure to change this for your skill.
    ...
    "en-US": {
        "translation": {
            ...
            "GAME_NAME" : "Know Your World language Trivia Challenge" // Be sure to change this for your skill.
        }
    }
};
...
function refineAnswers(answers)
{
...
function findItem(array, inputItem)
{
    var n = {},r=[];
...
function loadQuestions(noOfQuestions)
{
    var qandaCollection = [];
    for(var quesCtr = 0; quesCtr < noOfQuestions ; quesCtr++)
    {
    var qanda = {}; 
    ...
    var numberOfCountries = 0;
        for(var ctryCtr=0; ctryCtr<4 ; ctryCtr++)
        {
            ...
            if(countries.countries[randomNumber].languages.length>1)
            {
                selectedCountries.push(countries.countries[randomNumber]);
                numberOfCountries = numberOfCountries +1;
            }
        }
    var languageText = "Name the Languages of the Country " ;
    var answers = [];
            for(var langCtr= 0; langCtr< numberOfCountries; langCtr++)
            {
                var language = selectedCountries[langCtr].languages;
                console.log('language is ', language);
                if(language.indexOf(',')==-1)
                {
                    var languageDescription = languages[language].name;
                    answers.push(languageDescription);
                }
                else
                {
                    var splitVal = language.split(",")[0];
                    console.log(splitVal);
                    var languageDescriptionWithMultiple = languages[splitVal].name;
                    answers.push(languageDescriptionWithMultiple);
                }


            }
            console.log('answer collection is now:- ', answers);
    answers = refineAnswers(answers);     
            console.log('answer collection after refinement:- ', answers);
   
    qanda[languageText + selectedCountries[0].name] = answers;
    qandaCollection.push(qanda);
    }

    return qandaCollection;
}
